...
class EmbeddingLayer(nn.Module):
    def __init__(self, n_in, n_out):
        ...
        self.mask = nn.Sequential(nn.Linear(n_out, n_out),
                                  nn.BatchNorm1d(n_out),
                                  nn.ReLU(inplace=True))  # Fully connected layer to generate mask logits
        
        
    def gumbel_sigmoid(self, embeds, tau=2.0, hard=False):
        
        masks = self.mask(embeds)
        gumbels = -torch.log(-torch.log(torch.rand_like(masks) + 1e-20) + 1e-20)
        y = masks + gumbels
    ...
    def forward(self, c):
        ...
        gumbel_mask = self.gumbel_sigmoid(c_emb, hard=False)
        return (c_emb * gumbel_mask)
