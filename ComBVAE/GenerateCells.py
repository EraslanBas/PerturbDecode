...
def GenerateCells(n_cond_in, n_inputs, n_cond, n_latents, save_dir, model_dir, model_type, controlfile, batch_size, num_workers, project_dir, use_gpu):
    ...
    myconditions = dataset.get_targets()
    
    print("Aloooo")
    print(myconditions)
    
    target_conditions_dict = {}
    for i in range(0,num_conditions,1):
        for j in range(i,num_conditions,1):
    ...
    os.makedirs(os.path.join(save_dir, model_dir, 'generated_samples'),
                exist_ok=True)
